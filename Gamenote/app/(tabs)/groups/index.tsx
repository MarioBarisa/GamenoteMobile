    ...
    >
      {groups.length === 0 ? (
        ...
        groups.map((group) => {
          ...
          return (
            <View key={group.id} style={[styles.card, { backgroundColor: t.card }]}>
              <View style={styles.headerRow}>
                <View style={styles.titleSection}>
                  <Text style={{ color: t.text, fontWeight: "700", fontSize: 18 }}>{group.name}</Text>
                  {group.type && (
                    <Text style={{ color: t.secondaryText, fontSize: 12, marginTop: 2 }}>{group.type}</Text>
                  )}
                </View>
                {typeof group.rating === 'number' ? (
                  <View style={styles.ratingRow}>
                    {[1, 2, 3, 4, 5].map(star => (
                      <Ionicons
                        key={star}
                        name={star <= group.rating! ? 'star' : 'star-outline'}
                        size={14}
                        color={star <= group.rating! ? '#FF9F0A' : t.secondaryText}
                        style={{ marginRight: 1 }}
                      />
                    ))}
                  </View>
                      ...
                      >
                        {game?.image_url?.[0] ? (
                          ...
                          <View style={[
                            styles.gameImage,
                            isSingle && styles.gameImageSingle,
                            { backgroundColor: t.background, justifyContent: 'center', alignItems: 'center' }
                          ]}>
                            <Ionicons name="image-outline" size={isSingle ? 24 : 14} color={t.secondaryText} />
                          </View>
                        )}
                      </View>
                    ))}
